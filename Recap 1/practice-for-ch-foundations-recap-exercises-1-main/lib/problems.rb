...
def all_vowel_pairs(words)
  ...
  words.each_with_index do |word, index|
    (index +1... words.size).each do |i|
      w = words[i]
      if vow.all? {|j| word.include?(j) && w.include?(j)}
        res.append("#{word} #{w}")
      end
    end
  end
...
def find_bigrams(str, bigrams)
  bigrams.select {|i| str.include?(i)}
...
class Hash
  def my_select(&prc)
    ...
    self.each {|i,j| res[i] = j if prc.call(i,j)}
    res 
  end
end
  ...
  def substrings(length = nil)
    ...
    (0...self.length). each do |i|
      (i...self.length).each do |j|
        subs = self[i..j]
        if length.nil? || subs.length == length
          res.append(subs)
        end
      end
    end
